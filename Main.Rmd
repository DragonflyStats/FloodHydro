...
of argument specification. prior allows specify  arguments
 as expression without quotation
...
itself.
Below, we explain its 
#### Usage
<pre><code> and list some common prior distributions for parameters. A complete
...
#### Arguments
 lb and ub). The default prior is flat over the definition area.
...
... Additional </code></pre>
#### Arguments
 passed to the plotting functions. See MCMC-overview for
more #### Details.
#### Details
...
kappa Vector of precisionvalues.
log Logical; If TRUE,values are returned on the log scale.
lower.tail Logical; If TRUE (default), return P(X <= x). Else, return P(X > x) .
log.p Logical; If TRUE,values are returned on the log scale.
